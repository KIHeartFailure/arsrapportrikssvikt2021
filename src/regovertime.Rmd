```{r regovertime, cache=cacheon, fig.cap="Årlig registrering", fig.width=9.5, fig.height=7}

tot <- rsdata %>%
  filter(indexyear %in% paste(seq((ar - 6), ar, 1))) %>%
  count(indexyear) %>%
  mutate(
    byvar = 5,
...
hfdur <- rsdata %>%
  filter(indexyear %in% paste(seq((ar - 6), ar, 1)), ttype == "Index") %>%
  filter(!is.na(hfdur)) %>%
  group_by(indexyear) %>%
  count(hfdur) %>%
  mutate(
...
matplot(plotall,
  ...
  #type = "l",
  pch = 19,
  col = c(global_colsblue[1], global_cols[2], global_cols[2], global_colsblue[2:4], global_cols[3], global_cols[3]),
  lty = 1,
  lwd = 3,
  # cex = 1.2,
  axes = FALSE,
  xaxs = "i",
  yaxs = "i",
  xlim = c(1 - 0.2, length((ar - 6):ar) + 0.2),
...
axis(2,
  at = all %>% pull(!!sym(paste(ar))) / 100 + c(0, -2.2, 1.5, +1.5, -1.5, +2.2, 0, 0),
...
```{r regovertimetab, cache=cacheon, dependson="regovertime"}
